...
const HIGHLIGHTS = [
  { label: 'ROOFTOP', img: '/images/mila-interior.webp' },
  { label: 'OMAKASE', img: '/images/mila-omakase.webp' },
  { label: 'LOUNGE', img: '/images/mila-lounge.webp' },
  { label: 'EVENTS', img: '/images/mila-design.webp' },
  { label: 'MM CLUB', img: '/images/mila-mmclub.webp' },
  { label: 'PRESS', img: '/images/mila-feed1.webp' },
];

const GRID_PHOTOS = [
  '/images/mila-feed1.webp',
  '/images/mila-interior.webp',
  '/images/mila-feed2.webp',
  '/images/mila-omakase.webp',
  '/images/mila-feed3.webp',
  '/images/mila-lounge.webp',
  '/images/mila-design.webp',
  '/images/mila-mmclub.webp',
  '/images/mila-feed4.webp',
];

export function InstagramProfile({ onNext, autoPlay }: Props) {
  const [showGlow, setShowGlow] = useState(false);
  const [showTap, setShowTap] = useState(false);

  useEffect(() => {
    if (!autoPlay) return;
    const t1 = setTimeout(() => setShowGlow(true), 1500);
    const t2 = setTimeout(() => setShowTap(true), 3200);
  ...
  return (
    <div className="bg-white text-black min-h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-1.5 bg-white">
        ...
        <div className="flex items-center gap-1">
          <span className="text-sm font-semibold text-[#262626]">milagroup_miami</span>
      ...
      <div className="px-4 pt-3">
        <div className="flex items-start gap-5">
          ...
          <div className="flex gap-4 flex-1 justify-around pt-2">
            {[
              { num: '1,116', label: 'posts' },
              { num: '215K', label: 'followers' },
        ...
        {/* Name & bio */}
        <div className="mt-3">
          <div className="text-[13px] font-semibold text-[#262626]">MILA Miami Beach</div>
          ...
          <div className="flex items-center gap-1.5 mt-1.5 relative">
            ...
            <motion.button
              onClick={onNext}
              className="text-[13px] font-medium text-[#00376B] relative"
              whileTap={{ scale: 0.95 }}
              animate={autoPlay && showTap ? { scale: [1, 0.92, 1] } : {
                textShadow: [
              ...
              transition={autoPlay && showTap ? { duration: 0.3 } : { duration: 2, repeat: Infinity }}
            >
              link.me/mila
              <PreTapGlow show={showGlow && !showTap} />
              <TapIndicator show={showTap} />
            </motion.button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 mt-3">
          <div className="flex -space-x-2">
            <div className="w-[18px] h-[18px] rounded-full border border-white overflow-hidden bg-gray-200" />
            <div className="w-[18px] h-[18px] rounded-full border border-white overflow-hidden bg-gray-300" />
          ...
          <div className="text-[11px] text-[#262626]">
            Followed by <span className="font-semibold">friends</span> + 84 more
          </div>
        </div>
      ...
      <div className="sticky bottom-0 bg-white border-t border-[#DBDBDB] flex items-center justify-around py-2.5 px-4 mt-auto">
        ...
        <div className="w-6 h-6 rounded-full overflow-hidden border border-[#DBDBDB] bg-gray-200" />
      </div>
    </div>
  );
}
